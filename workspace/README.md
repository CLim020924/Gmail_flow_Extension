...
검증된 `CMOE-Workspace-Setup-1.1.15-Windows.exe`를 실행합니다. 설치형은 바탕화면과 시작 메뉴에 바로가기를 만들며, 사용자 데이터는 프로그램 업데이트 후에도 유지됩니다. 현재 설치 파일에는 상용 코드 서명 인증서가 적용되지 않아 다른 PC의 Windows SmartScreen이 게시자 경고를 표시할 수 있습니다. 이 저장소는 아직 설치 파일을 GitHub Releases에 자동 게시하지 않으므로, 배포 담당자가 smoke 검사를 통과한 설치 파일을 별도로 전달하거나 수동으로 게시해야 합니다.
